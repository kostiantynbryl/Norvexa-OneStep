    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (existing == null) {
                ...
                item { Text(stringResource(R.string.template_hint), color = MaterialTheme.colorScheme.onSurfaceVariant) }
            }
            item {
                Button(
                    onClick = {
                        if (title.isBlank()) {
                            ...
                            onCreate(title, description, category, dailyMinutes.toIntOrNull() ?: 25, deadlineDays.toIntOrNull() ?: 30)
                        } else {
